...
void Game_Connect4_Bitboards::Display() const
{
    std::cout << GetDisplayString();
}

void Game_Connect4_Bitboards::DisplayActionSequence() const
{
    std::cout << GetDisplayActionSequenceString();
}

std::string Game_Connect4_Bitboards::GetDisplayString() const
{
    std::string s;
...
std::string Game_Connect4_Bitboards::GetDisplayActionSequenceString() const
{
    std::string result;
...
bool Game_Connect4_Bitboards::IsValidAction(int Action) const
{
    const int i = Action;
...
void Game_Connect4_Bitboards::Do(int Action)
{
...
void Game_Connect4_Bitboards::Undo(int Action)
{
    TurnNumber--;
...
bool Game_Connect4_Bitboards::IsValidCell(std::uint_fast64_t i) const
{
...
bool Game_Connect4_Bitboards::IsValidPlayer(int p) const
{
    return p == 1 || p == 2;
}

std::uint_fast64_t Game_Connect4_Bitboards::WinningPieceMask() const
{
    //calculate a msk for winning any pieces so they can be capitalised
...
Game_Connect4_Bitboards::Game_Connect4_Bitboards()
{
...
bool Game_Connect4_Bitboards::IsWinningState() const
{
...
std::string Game_Connect4_Bitboards::GetName() const
{
    return "Connect4";
}

int Game_Connect4_Bitboards::GetActivePlayer() const
{
    assert(IsValidPlayer(ActivePlayer));
    return ActivePlayer;
}

int Game_Connect4_Bitboards::GetTurnNumber() const
{
    assert(TurnNumber >= 0 && TurnNumber <= 42);
    return TurnNumber;
}

void Game_Connect4_Bitboards::GetValidActions(std::vector<int>& OutActions) const
{
...
Game::PlayState Game_Connect4_Bitboards::GetPlayState() const
{
    if (IsWinningState())
    {
...
void Game_Connect4_Bitboards::Reset()
{
    TurnNumber = 0;
    ActivePlayer = 1;
    ActivePlayerOwned = 0;
...
std::vector<int> Game_Connect4_Bitboards::GetStateVector() const
{
    ...
    {
        arr.push_back(ActivePlayerOwned >> 8 * sizeof(int) * i);
    }
    for (int i = 0; i < sizeof(std::uint_fast64_t); i += sizeof(int))
    {
        arr.push_back(InactivePlayerOwned >> 8 * sizeof(int) * i);
    }
